 ...
 * 不持久化完整 minimalComplete / showDeveloperPanel；可持久化 structureModuleId（结构文件名 id）。
 */

import type { StructureData } from '@/render/types'
import type { AppPreviewConfig } from './appPreviewConfig'
import {
  getExportBlockRegistryByModuleId,
  getExportMaterialRegistryByModuleId,
  getStructureDataByModuleId,
} from './structureModuleCatalog'
...
export function mergeAppPreviewConfig(base: AppPreviewConfig, patch: Partial<AppPreviewConfig>): AppPreviewConfig {
  let minimalComplete = patch.minimalComplete ?? base.minimalComplete
  let structureModuleId = base.structureModuleId

  if (patch.structureModuleId !== undefined) {
    if (patch.structureModuleId === '') {
      minimalComplete = base.minimalComplete
      structureModuleId = undefined
    } else {
      const data = getStructureDataByModuleId(patch.structureModuleId)
      if (data !== undefined) {
        minimalComplete = {
          structure: data as StructureData,
          blockRegistry:
            getExportBlockRegistryByModuleId(patch.structureModuleId) ?? base.minimalComplete.blockRegistry,
          materialRegistry:
            getExportMaterialRegistryByModuleId(patch.structureModuleId) ?? base.minimalComplete.materialRegistry,
        }
        structureModuleId = patch.structureModuleId
      } else {
        minimalComplete = patch.minimalComplete ?? base.minimalComplete
  ...
  return {
    ...
    minimalComplete,
    structureModuleId,
    devGlobalBlockRegistry: patch.devGlobalBlockRegistry ?? base.devGlobalBlockRegistry,
